#' Multiple Imputation through Indirect Use of Regularized Regression(IURR)in the presence of General missing pattern
#' 
...
GMiurr=function(data,m=5,family=rep("gaussian",3),method=c("lasso"),included=FALSE,burn=20){
ar1<-function(nn,rho){
...
durr.pred=function(z_obs,W_obs,W_mis,family=c("gaussian"),method=c("lasso")){
  ...
  for(mm in 1:m){
    ...
    for(j in 1:l){
      W=Z1[,-ind.col[j]]
      aset=tryCatch(act.set.dy(z_obs=data[-ind.row[[j]],ind.col[j]],W_obs=W[-ind.row[[j]],],family=family[j],method=method),error=function(e) NULL)
      if(is.list(aset)==FALSE || aset$nochange==TRUE){
        imp[[j]]=cbind(imp[[j]],imp[[j]][,ncol(imp[[j]])])
        next}
      
      if(family[j]=="gaussian"){
        newreg=tryCatch(lm(data[-ind.row[[j]],ind.col[j]]~W[-ind.row[[j]],aset$aset]),error=function(e) NULL)
        if(length(newreg)==0){
          imp[[j]]=cbind(imp[[j]],imp[[j]][,ncol(imp[[j]])])
          next}
        theta=c(mvrnorm(n=1,as.vector(coef(newreg)),vcov(newreg)),sum(newreg$residuals^2)/(n-length(ind.row[[j]])-length(aset$aset)-1))
        imp[[j]]=cbind(imp[[j]],impute.dy(W_mis=W[ind.row[[j]],aset$aset],theta,family=c("gaussian")))  
      } 
      
      if(family[j]=="binary"){
        newreg=tryCatch(glm(data[-ind.row[[j]],ind.col[j]]~W[-ind.row[[j]],aset$aset]),error=function(e) NULL)
        if(length(newreg)==0){
          imp[[j]]=cbind(imp[[j]],imp[[j]][,ncol(imp[[j]])])
          next}
        theta=unname(mvrnorm(n=1,as.vector(coef(newreg)),vcov(newreg)))
        imp[[j]]=cbind(imp[[j]],impute.dy(W_mis=W[ind.row[[j]],aset$aset],theta,family=c("binary")))  
      }  
      ...
      if(family[j]=="poisson"){
        ...
        if(length(newreg)==0){
          ...
          next}
        theta=unname(mvrnorm(n=1,as.vector(coef(newreg)),vcov(newreg)))
        imp[[j]]=cbind(imp[[j]],impute.dy(W_mis=W[ind.row[[j]],aset$aset],theta,family=c("poisson")))  
      }   
      
      Z1[ind.row[[j]],ind.col[[j]]]=imp[[j]][,mm+1]
